...
def battlegame():
    ...
    #Load sounds
    sound_gethurt = pygame.mixer.Sound("art/Sound_gethurt.wav")
    sound_getscore = pygame.mixer.Sound("art/Sound_score.wav")
    sound_debuff = pygame.mixer.Sound("art/Sound_debuff.wav")
    ...
    player_speed = length*0.01
    lower_speed = length*0.004
    flag_debuf = False
    time_buff_now = 0
    buff_time = 3000
    ...
    while running:
        ...
        if flag_debuf:
            ...
            if (keys[pygame.K_a] or keys[pygame.K_LEFT]) and px > 0 and playing:
                        px -= player_speed
        screen.blit(player_image,(px,py))    
        if player_immune_time and not player_dead:
            if time_used%2 == 0:
